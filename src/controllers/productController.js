...
export const getProducts = catchAsyncError(async (req, res, next) => {
    ...
    let productsGot = products.length
    ...
    res.status(200).json({
        success: true,
        productsGot,
        pageCount: Math.ceil(productsGot / resultsPerPage),
        productCount,
        resultsPerPage,
...
// Get Featured Product => /api/v1/products/featured

export const getFeaturedProducts = catchAsyncError(async (req, res, next) => {
    const products = await Product.find({ price: { $gte: 500 } }).limit(10)
...
// Get Top Products => /api/v1/products/top
